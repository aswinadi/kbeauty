<x-filament-panels::page>
    ...
    {{-- TRACER BULLET: REMOVE AFTER VERIFICATION --}}
    <div class="p-4 mb-4 text-sm text-white bg-red-600 rounded-lg dark:bg-red-500" role="alert">
        <span class="font-medium">UPDATE VERIFICATION V5:</span> If you see this red box, the code deployment WAS SUCCESSFUL. The layout below includes <code>table-fixed</code> and inline icon styles.
    </div>
                        ...
                        <tbody>
                            @forelse ($data['data'] ?? [] as $row)
                                <tr
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-900 dark:text-white truncate" title="{{ $row['location_name'] }}">
                                        {{ $row['location_name'] }}
                                    </td>
                ...
                @else
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-fixed">
                        ...
                        @forelse ($movements as $movement)
                            ...
                            @endphp
                            <tr class="border-b dark:border-gray-700 odd:bg-white even:bg-gray-50 dark:odd:bg-gray-800 dark:even:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition duration-150">
                                {{-- Date Column --}}
                                ...
                                <td class="px-6 py-4 align-top">
                                    <div class="flex flex-col gap-1">
                                        <div class="font-medium text-gray-900 dark:text-white">
                                            ...
                                            <span class="text-xs text-gray-500 uppercase tracking-wider border border-gray-200 rounded px-1.5 py-0.5">
                                        ...
                                        <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 mt-1">
                                            <div class="flex items-center gap-1 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded text-xs">
                                                <x-heroicon-m-map-pin class="w-4 h-4 text-gray-500" style="width: 1rem; height: 1rem;" />
                                                {{ $locationLabel }}
                                            </div>
                                            <span class="text-gray-300">|</span>
                                            <div class="flex items-center gap-1 text-xs">
                                                <x-heroicon-m-user class="w-4 h-4 text-gray-500" style="width: 1rem; height: 1rem;" />
                                ...
                                {{-- Balance Column --}}
                                <td class="px-6 py-4 text-right align-top font-bold text-gray-900 dark:text-white whitespace-nowrap">
                                    {{ number_format($balance, 0) }}
                                </td>
                            </tr>
                        @empty
